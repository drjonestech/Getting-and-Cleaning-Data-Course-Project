...
#set directory
setwd("C:/Users/CrisWin/Documents/Data Science/Getting and Cleaning Data Wk4/hardata/HAR Dataset");
...
## Modify column names for test datasets
colnames(subjectTest) <-"subjectId";
colnames(xTest)       = features[,2]; 
colnames(yTest)       = "activityId";


## m\Merge xTest, yTest and subjectTest 
...
for (i in 1:length(colNames)) 
{
  ...
  colNames[i] = gsub("([Bb]odyaccjerkmag)","BodyAccjJerkmagnitude",colNames[i])
...
tidyData    = aggregate.data.frame(finalDataNoActivityType[,names(finalDataNoActivityType) != c('activityId','subjectId')],by=list(activityId=finalDataNoActivityType$activityId,subjectId = finalDataNoActivityType$subjectId),mean);
